...
/** Reads the first text block from an `error()` tool result. */
function errorText(result: unknown): string {
  const block = (result as { content: { type?: string; text?: string }[] }).content[0]
...
describe("withToolErrors", () => {
  ...
  it("converts a coded exception to the registrar's `[code] message` error result", async () => {
    ...
    expect(errorText(result)).toBe("[ENGINE_NOT_SELECTED] No engine selected for this session.")
  })

  it("prefers an HTTP-ish `status` over `code`, matching the registrars", async () => {
    const e = Object.assign(new Error("boom"), { status: 404, code: "IGNORED" })
    const wrapped = withToolErrors(() => Promise.reject(e))
    expect(errorText(await wrapped())).toBe("[404] boom")
  })
