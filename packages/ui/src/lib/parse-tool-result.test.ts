...
describe("parseToolResult (structuredContent-first contract)", () => {
  ...
  it("treats structuredContent: null as absent and falls through to text", () => {
    const result = {
      content: [{ type: "text", text: JSON.stringify({ fromText: true }) }],
      structuredContent: null,
    }
    expect(parseToolResult(result)).toEqual({ fromText: true })
  })

  it("returns a falsy-but-present structuredContent (e.g. 0) instead of falling through", () => {
    const result = { content: [{ type: "text", text: "summary" }], structuredContent: 0 }
    expect(parseToolResult<number>(result)).toBe(0)
  })
...
describe('parseToolResult with prefer: "text"', () => {
  it("reads the text channel first when both are present", () => {
    const result = {
      content: [{ type: "text", text: JSON.stringify({ fromText: true }) }],
      structuredContent: { fromStructured: true },
    }
    expect(parseToolResult(result, { prefer: "text" })).toEqual({ fromText: true })
  })

  it("falls back to structuredContent when there is no text block", () => {
    const result = { structuredContent: { fromStructured: true } }
    expect(parseToolResult(result, { prefer: "text" })).toEqual({ fromStructured: true })
  })

  it("returns non-JSON text verbatim before consulting structuredContent", () => {
    const result = {
      content: [{ type: "text", text: "plain line" }],
      structuredContent: { fromStructured: true },
    }
    expect(parseToolResult<string>(result, { prefer: "text" })).toBe("plain line")
  })

  it("still throws on isError results", () => {
    const result = { isError: true, content: [{ type: "text", text: "boom" }] }
    expect(() => parseToolResult(result, { prefer: "text" })).toThrowError("boom")
  })
})
