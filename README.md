...
`@miragon/mcp-toolkit-core` peer-deps `proxy-contract`, so it must be installed alongside `core`. The peer dependencies are pinned exactly — install `mcp-use@1.32.1`, `@modelcontextprotocol/sdk@1.29.0`, and `zod@4.4.3` alongside (see each package's `peerDependencies` for the authoritative list).
